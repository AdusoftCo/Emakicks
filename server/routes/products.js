    ...
    if (search) {
      ...
      conditions.push(`
        (
          ...
          OR p.cod_art ILIKE $${params.length}
          OR f.nombre ILIKE $${params.length}
        )
      `);
    }
...
router.get("/libreta", async (req, res) => {
  const result = await pool.query(
    "SELECT * FROM libreta ORDER BY fecha DESC"
  );
  res.json(result.rows);
});

router.post("/libreta", async (req, res) => {
  const { novedad, monto, fecha } = req.body;

  await pool.query(
    "INSERT INTO libreta (novedad, monto, fecha) VALUES ($1,$2,$3)",
    [novedad, monto, fecha || new Date()]
  );
